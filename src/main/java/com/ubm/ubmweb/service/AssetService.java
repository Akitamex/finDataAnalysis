...
public class AssetService {
    ...
    @Transactional
    public Asset createAsset(CreateAssetInput input, UUID userId){
    ...
    @Transactional
    public Asset updateAsset(UUID userId, UpdateAssetInput input){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public void deleteAssetById(UUID userId, UUID companyId, UUID id) {
    ...
    public List<Asset> findAssets(UUID userId, UUID companyId, String name, Long quantity, BigDecimal remainingCost, BigDecimal wholeCost) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated) {
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional(readOnly = true)
    public Asset getAssetById(UUID userId, UUID id, UUID companyId) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated) {
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
