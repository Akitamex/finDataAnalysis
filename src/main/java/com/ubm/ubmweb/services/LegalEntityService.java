...
public class LegalEntityService {
    ...
    @Transactional
    public LegalEntity createLegalEntity(Long userId, CreateLegalEntityInput input){
    ...
    public List<LegalEntity> findLegalEntities(Long userId, Long companyId, String name, String fullName, String IIN, String COR, String MSRN, String legalAddress, String phoneNum, Boolean VAT) {
    ...
    @Transactional(readOnly = true)
    public LegalEntity getLegalEntityByIdAndCompanyId(Long userId, Long companyId, Long id){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public LegalEntity updateLegalEntity(Long userId, UpdateLegalEntityInput input){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public void deleteLegalEntity(Long userId, Long companyId, Long id){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
