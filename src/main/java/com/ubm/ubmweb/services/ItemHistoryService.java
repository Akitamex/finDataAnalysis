package com.ubm.ubmweb.services;        //История отгрузок товарных запасов
...
public class ItemHistoryService {
    ...
    @Transactional
    public ItemHistory createItemHistory(Long userId, CreateItemHistoryInput input) {
    ...
    @Transactional
    public ItemHistory updateItemHistory(Long userId, UpdateItemHistoryInput input) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
        if (!isAssociated) {
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public void deleteById(Long userId, Long companyId, Long id) {
    ...
    @Transactional(readOnly = true)
    public List<ItemHistory> findItemHistories(Long userId, Long companyId, Long itemId, DateRangeInput dateRange,
                                               List<Long> counterpartyIds, List<Long> legalEntityIds, Boolean isIncoming) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional(readOnly = true)
    public ItemHistory getItemHistoryById(Long userId, Long id, Long companyId) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
