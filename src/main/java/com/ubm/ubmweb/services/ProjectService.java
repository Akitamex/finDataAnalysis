...
public class ProjectService {
    ...
    @Transactional
    public Project createProject(Long userId, CreateProjectInput input){
    ...
    public List<Project> findProjects(Long userId, Long companyId, List<String> types, List<String> statuses, List<Long> directionIds, String name, String description) {
    ...
    @Transactional(readOnly = true)
    public Project getProjectByIdAndCompanyId(Long userId, Long companyId, Long id){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public Project updateProject(Long userId, UpdateProjectInput input){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public void deleteProject(Long userId, Long companyId, Long id){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
