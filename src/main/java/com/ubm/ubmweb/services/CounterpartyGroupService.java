...
public class CounterpartyGroupService {
    ...
    @Transactional
    public CounterpartyGroup createCounterpartyGroup(Long userId, Long companyId, String name){
    ...
    public List<CounterpartyGroup> findCounterpartyGroups(Long userId, Long companyId, String name) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional(readOnly = true)
    public CounterpartyGroup getCounterpartyGroupByIdAndCompanyId(Long userId, Long companyId, Long id){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public CounterpartyGroup updateCounterpartyGroup(Long userId, Long companyId, Long id, String name){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public void deleteCounterpartyGroup(Long userId, Long companyId, Long id){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
