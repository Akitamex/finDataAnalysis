...
public class FixedAssetService {
    ...
    @Transactional
    public FixedAsset updateFixedAsset(Long userId, UpdateFixedAssetInput input) {
        if (input.getQuantity() < 1) {
            throw new IllegalArgumentException("Fixed Asset Quantity should be >= 1");
        }
        ...
        Long assetId = fixedAsset.getAsset().getId();
    ...
    @Transactional
    public void deleteById(Long id, Long userId, Long companyId) {
    ...
    public List<FixedAsset> findFixedAssets(Long userId, Long companyId, Boolean amortise, String name, LocalDate purchaseDate,
    ...
    @Transactional(readOnly = true)
    public FixedAsset getFixedAssetById(Long userId, Long id, Long companyId) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated) {
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    public void updateRemainingCosts() {
        ...
        LocalDate today = LocalDate.now();
        Map<Long, BigDecimal> assetRemainingCosts = new HashMap<>();
        ...
        for (Map.Entry<Long, BigDecimal> entry : assetRemainingCosts.entrySet()) {
