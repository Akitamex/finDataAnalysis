...
public class ArticleService {
    ...
    @Transactional
    public Article createArticle(Long userId, CreateArticleInput input){
    ...
    public List<Article> findArticles(Long userId, Long companyId, String name, List<Long> articleGroupIds, List<String> types, String description, Integer category) {
    ...
    @Transactional(readOnly = true)
    public Article getArticleByIdAndCompanyId(Long userId, Long id, Long companyId){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public Article updateArticle(Long userId, UpdateArticleInput input){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public void deleteArticle(Long id, Long userId, Long companyId){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
