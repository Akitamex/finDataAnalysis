...
public class OperationsService {
    ...
    @Transactional
    public IncomeOperation createIncomeOperation(Long userId, CreateIncomeOperationInput input){
    ...
    @Transactional
    public ExpenseOperation createExpenseOperation(Long userId, CreateExpenseOperationInput input){
        if(input.getBalance().compareTo(BigDecimal.ZERO) < 1){
            throw new IllegalArgumentException("Balance can not be set to less than 0");
        }
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
    ...
    @Transactional
    public TransferOperation createTransferOperation(Long userId, CreateTransferOperationInput input){
        if(input.getBalance().compareTo(BigDecimal.ZERO) < 1){
            throw new IllegalArgumentException("Balance can not be set to less than 0");
        }
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
    ...
    public Operation getOperationByIdAndCompanyId(Long userId, Long id, Long companyId){
    ...
    @Transactional(readOnly = true)
    public List<Operation> findOperations(Long userId, Long companyId, DateRangeInput dateRange, List<Long> articleIds, 
    ...
    @Transactional
    public IncomeOperation updateIncomeOperation(Long userId, UpdateIncomeOperationInput input){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public ExpenseOperation updateExpenseOperation(Long userId, UpdateExpenseOperationInput input){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public TransferOperation updateTransferOperation(Long userId, UpdateTransferOperationInput input){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(input.getCompanyId(), userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public void deleteOperation(Long id, Long userId, Long companyId){
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
