...
public class ObligationService {
    ...
    @Transactional
    public Obligation createObligation(UUID userId, CreateObligationInput input) {
    ...
    public List<Obligation> findObligations(UUID userId, UUID companyId, DateRangeInput dateRange, List<String> types,
                                            List<UUID> counterpartyIds, List<UUID> legalEntityIds, List<UUID> projectIds, String description) {
    ...
    @Transactional(readOnly = true)
    public Obligation getObligationById(UUID userId, UUID id, UUID companyId) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated){
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public Obligation updateObligation(UUID userId, UpdateObligationInput input) {
        UUID obligationId = input.getId();
        ...
        UUID companyId = obligation.getCompany().getId();
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated) {
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
    ...
    @Transactional
    public void deleteObligation(UUID userId, UUID companyId, UUID id) {
        boolean isAssociated = userCompanyRelationshipRepository.existsById(new UserCompanyId(companyId, userId));
        if (!isAssociated) {
            throw new UnauthorizedAccessException("User does not have access to update this company.");
        }
